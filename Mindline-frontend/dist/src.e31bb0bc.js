...
class OrdersView {
  ...
  async getOrders() {
    try {
      ...
      console.log("orders", userOrders.order);
      this.render();
    } catch (err) {
      _Toast.default.show(err, 'error');
    }
...
class App {
  constructor() {
    ...
    this.version = "1.0.0";
    this.apiBase = "https://cafe-minori-app-4d4dfdb9bbc1.herokuapp.com";
...
customElements.define('va-order', class Order extends _litElement.LitElement {
  ...
  async addFavHandler() {
    try {
      await _UserAPI.default.addFavProducts(this.id);
      _Toast.default.show('Product added to favourites');
    } catch (err) {
      _Toast.default.show(err, 'error');
    }
